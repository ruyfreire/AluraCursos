class AutoresDao {
    ...
    listarPorArrayId(autoresId) {
        return new Promise((resolve, reject) => {
            this._db.all(`SELECT id, nome FROM autores WHERE id IN (${ autoresId.map(nome => nome ).join(',') })`,
            (err, rows) => {
                if(err) {
                    console.log(err);
                    return reject('Erro ao buscar autor do livro');
                }
                resolve(rows);
            });
        });
